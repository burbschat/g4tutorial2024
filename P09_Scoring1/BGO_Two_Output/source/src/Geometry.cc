...
//------------------------------------------------------------------------------
{
  ...
  G4double leng_Z_BGO = 10.0 * cm;
  auto solid_BGO = new G4Tubs{ "Solid_BGO",      0., radius_BGO,
                               leng_Z_BGO / 2.0, 0., 360. * deg };
  ...
  G4double pos_Z_LogV = -5.0 * cm; // Z-location LogV
  ...
  G4int copyNum_LogV = 1001; // Set ID number of LogV
  ...
  pos_Z_LogV = +5.0 * cm; // Z-location LogV
  ...
  // Sensitive volume
  SensitiveVolume* aSV = new SensitiveVolume{ "BGO" };
  aSV->SetSimData (simdata);
  auto SDman = G4SDManager::GetSDMpointer ();
  SDman->AddNewDetector (aSV);
  logVol_BGO->SetSensitiveDetector (aSV);
