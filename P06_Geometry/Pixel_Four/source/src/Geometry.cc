...
//------------------------------------------------------------------------------
{
  ...
  auto solid_World = new G4Box{ "Solid_World", leng_X_World / 2.0,
  ...
  auto physVol_World = new G4PVPlacement{
  ...
  G4double leng_X_PixEnvG
      = 15.0 * mm; // X-full-length of pixel: global envelop
  G4double leng_Y_PixEnvG
      = 20.0 * mm;                    // Y-full-length of pixel: global envelop
  ...
  new G4PVReplica{
    "PhysVol_PixEnvL", logVol_PixEnvL, logVol_PixEnvG, kYAxis, nDiv_Y,
    leng_Y_PixEnvL
  };
  ...
  new G4PVReplica{
    "PhysVol_PixElmt", logVol_PixElmt, logVol_PixEnvL, kXAxis, nDiv_X,
    leng_X_PixElmt
  };

  // Placement of the 'Pixel Detectors' to the world: Put the 'global envelop'
  // Z positions and setting angles of four detectors
  G4double z_pos[4] = { -5.0 * cm, -2.5 * cm, +2.5 * cm, +5.0 * cm };
  G4double z_ang[4] = { 0.0 * deg, -45.0 * deg, +45.0 * deg, 0.0 * deg };
  ...
  for (G4int id = 0; id < 4; id++)
    {
      ...
      auto threeVect_LogV_PixEnvG
          = G4ThreeVector{ pos_X_LogV_PixEnvG, pos_Y_LogV_PixEnvG,
                           pos_Z_LogV_PixEnvG };
      ...
      new G4PVPlacement{ trans3D_LogV_PixEnvG,
                         "PhysVol_PixEnvG",
                         logVol_PixEnvG,
                         physVol_World,
                         false,
                         copyNum_LogV_PixEnvG,
                         true };
    }
