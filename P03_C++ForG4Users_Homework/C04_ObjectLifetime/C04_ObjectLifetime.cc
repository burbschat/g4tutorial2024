...
//
//        （注）
//        関数は渡されたオブジェクトのコピーをブロック内で作る。そして、この
...
// [課題 2] ポインタ渡しを使う
//    [問]
//    以下の関数'PassPoint()'は、ポインターで渡されたオブジェクトに定数'20'を加える。
//         この関数を実行すると、元のオブジェクトも変化していること確かめよ。
//------------------------------------------------------------------------------

void
PassPointer (float* val)
{
...
// [課題 3] リファレンス渡しを使う
//    [問]
//    以下の関数'PassReference()'は、レファレンスで渡されたオブジェクトに定数'30'を
//         加える。この関数を実行すると、元のオブジェクトも変化していること確かめよ。
//------------------------------------------------------------------------------

void
PassReference (float& val)
{
...
// [課題 4] 返り値渡しを使う
//    [問]
//    以下の関数'PassReturn()'は、渡されたオブジェクトに定数'40'を加える。そして加算
//         されたオブジェクトを'return'で返す。これで'RVO:Return Value
//         Optimization'が
//         作動。この関数を実行すると、関数外の元のオブジェクトも変化していること確かめよ。
//------------------------------------------------------------------------------

float
PassReturn (float val)
{
  return val
         += 40; // valは'RVO'でreturnされる：　ローカル・オブジェクトに'40'を加算
}

//------------------------------------------------------------------------------
// 以上で課題は終了　ー　以下は課題
// 1~4の[問]へ解答する際に参照する関数の実行プログラム　
//------------------------------------------------------------------------------
