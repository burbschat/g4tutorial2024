...
main ()
{
  ...
  ? ? ? ? ? ? mOb2 ? 2.71
      ? ; // [問] 上例と同じタイプのオブジェクトを作り初期化せよ

  //------------------------------------------------------------------------------
  // [課題 2]  標準ライブラリ(C++ Standard
  // Library)提供のタイプのオブジェクト作成方法
  //------------------------------------------------------------------------------
  ...
  //--- stringタイプが持つmember関数
  //"length()"を使用し、データの文字数を取得する
  ...
  ? ? ? ? ? ? <double> mOb4 ? 9.87,
      6.54 ? ; // [問] 上例と同じタイプのオブジェクトを作り初期化せよ
  ...
  //------------------------------------------------------------------------------
  // [課題 3]
  // '&演算子'を使ったオブジェクトのポインタ取得、及び'*演算子'の使用方法
  //------------------------------------------------------------------------------
  ...
  vector<double> evec
      = vector<double>{ 5.67, 6.78, 7.89 }; // vectorオブジェクトの作成
  vector<double> mvec
      = vector<double>{ 5.67, 6.78, 7.89 }; // vectorオブジェクトの作成
  ...
  vector<double> evecc
      = *evecp; // vectorオブジェクトへのポインタからvectorオブジェクトを取得
  vector<double> mvecc = ? mvecp; // [問] 上例と同じタイプのオブジェクトを作成

  // ポインタが指すオブジェクトのmember関数を使う
  int evecp_Size
      = evecp->size (); // vectorタイプの関数 "size()"を使い、要素の総数を取得
  int mvecp_Size = mvecp ? ? size (); // [問] 上例と同じように関数を使う

  double evecp_At
      = evecp->at (1); // vectorタイプの関数 "at()"を使い、2番目要素の値を取得
  double mvecp_At = mvecp ? ? at (1); // [問] 上例と同じように関数を使う

  double evec_At = evec.at (
      1); // ポインタからでなく直接オブジェクトから関数"at()"を使う場合
