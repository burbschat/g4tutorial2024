...
class Shape_2P
{       // [問] Shape_2Pクラス宣言
public: // [問] publicメンバー（データと関数）の定義
  string
  shape ()
  {
    return name;
...
protected: // [問] protectedメンバー（データと関数)の定義
...
//-----------------------------------------------------------------------------
// [課題 2]  Rectangular クラスを作る -
// Shape_2Pを継承し、constructorとarea()関数を実装
//-----------------------------------------------------------------------------

class Rectangular : public Shape_2P
{       // [問] Shape_2Pをpublic継承しRectangularを宣言
public: // publicメンバー（データと関数）の定義
  Rectangular (double x, double y)
  { // [問] Recutangularのconstructorの定義
    sizeX = x;
    sizeY = y;
    name = "Rectangular";
  }
...
{       // [問] Shape_2Pをpublic継承しEllipseを宣言
public: // publicメンバー（データと関数）の定義
  Ellipse (double x, double y)
  { // [問] Ellipseのconstructorの定義
    sizeX = x;
    sizeY = y;
    name = "Ellipse";
  }
...
main ()
{
  ...
  cout << "Shape: " << pict.shape ()
       << endl; // 'pict'が持つshape()関数へアクセス
  cout << "Area: " << pict.area ()
       << endl; // [問] 'pict'が持つarea()関数へアクセス

  Rectangular rect{ 3.0, 4.0 }; // 'Rectangular'のオブジェクトを作成
  cout << "Shape: " << rect.shape ()
       << endl; // 'rect'が持つshape()関数へアクセス
  cout << "Area: " << rect.area ()
       << endl; // '[問] 'rect'が持つarea()関数へアクセス

  Ellipse elps{ 5.0, 6.0 }; // 'Ellipse'のオブジェクトへを作成
  cout << "Shape: " << elps.shape ()
       << endl; // 'elps'が持つshape()関数へアクセス
  cout << "Area: " << elps.area ()
       << endl; // [問] 'elps'が持つarea()関数へアクセス
}
